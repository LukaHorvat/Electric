	...
	public class DebugConsole : DisplayObject
	{
		...
		public override void UpdateSelf()
		{
			...
			if (Input.Keys[Key.Enter] == InputState.Release)
			{
				...
				{
					builder.Append(pair.Value.GetType().FullName + " " + pair.Key + ", ");
				}
				var code = @"
using System;
using System.Collections.Generic;
...
namespace FireflyGL
{
	class DebugConsole
	{
		...
		{
			var types = Assembly.GetExecutingAssembly().GetTypes().Where(type => !type.Attributes.HasFlag(TypeAttributes.Public));
			if (types.Count() > 0) Console.WriteLine("The following types are not public:");
			foreach (var type in types)
			{
				if (type.Name.IndexOf("PrivateImplementationDetails") != -1) continue;
